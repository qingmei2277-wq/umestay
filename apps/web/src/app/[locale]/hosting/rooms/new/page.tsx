...
export default async function NewRoomPage({ params }: PageProps) {
  ...
  if (!user) redirect(`/${locale}/login?next=/${locale}/hosting/rooms/new`);
  ...
  return (
    <>
      <h1 className="text-xl font-bold text-gray-900 mb-6">{t("room_new")}</h1>
      <RoomForm locale={locale} labels={labels} saveRoom={saveRoom} />
    </>
  );
}
